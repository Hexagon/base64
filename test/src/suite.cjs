...
module.exports = function (base64) {
	...
	test("Encode array buffer", function () {
		let result = base64.fromArrayBuffer(new Uint8Array([0x0,0x1,0x2,0x3,0x4,0x5,0x6,0x7,0x8]));
		assert.equal(result, "AAECAwQFBgcI");
	});

	test("Decode 'Hello World'", function () {
		let result = base64.toArrayBuffer("AAECAwQFBgcI");
		assert.equal(result, new Uint8Array([0x0,0x1,0x2,0x3,0x4,0x5,0x6,0x7,0x8]));
	});
